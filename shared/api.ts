...
//
// Phase 2 schema (~22 questions). New required fields:
//   - dietaryPreference (was inferred from substring tests)
//   - energyPattern    (replaces tiredTime + energyLevels)
//   - gutSymptoms      (multi-select; replaces bloatingFrequency + digestiveIssues)
//
// Removed from new submissions: cravings, eatingOut, workSchedule, dnaUpload (binary).
// Those are kept as `.optional()` below so legacy stored submissions still validate
// and can be normalised by `analyzeQuizData` into the new fields.
export const WellnessQuizSchema = z.object({
  ...
  // Phase 2 — accept legacy-only stored payloads (these are derived from
  // tiredTime/energyLevels/digestiveIssues/bloatingFrequency in
  // quiz-analysis.ts when missing). Keep optional at the schema boundary.
  dietaryPreference: z.enum(["non-veg", "veg", "vegan", "eggetarian"]).optional(),
  ...
  gutSymptoms: z.array(
    z.enum(["acidity", "constipation", "loose-motions", "gas", "bloating", "none"]),
  ).optional(),
  ...
  // Legacy fields — kept optional so old stored submissions still validate.
  // analyzeQuizData() normalises these into the new fields above.
  tiredTime: z.enum(["morning", "afternoon", "evening", "rarely"]).optional(),
  bloatingFrequency: z.enum(["often", "sometimes", "rarely", "never"]).optional(),
  digestiveIssues: z.enum(["acidity", "constipation", "loose-motions", "gas", "none"]).optional(),
  cravings: z.enum(["sweet-foods", "salty-snacks", "fried-junk", "spicy-sour", "no-cravings"]).optional(),
  energyLevels: z.enum(["very-low", "low", "moderate", "high", "very-high"]).optional(),
  eatingOut: z.enum(["daily", "3-5-times", "1-2-times", "rarely"]).optional(),
  workSchedule: z.enum(["9-to-5", "shift-work", "flexible", "student", "homemaker"]).optional(),
  dnaUpload: z.enum(["yes-upload", "have-but-no-upload", "dont-have"]).optional(),
});
