...
import { validatePDFBundle, runPreGenerationChecks, filterFoodLine, getSleepWindow } from "../../shared/pdf-validation";
import { selectVariants, buildLabInterpretationGrid, type LabPanelRow } from "../../shared/decision-engine";
import { CONTENT_BLOCKS, getVariant, type SectionId, type VariantBlock } from "../../shared/content-blocks";
import { tierBadge } from "../../shared/contraindications";
...
export function getEssentialPathologyTests(profile: WellnessUserProfile): Array<{name: string; reason: string; frequency: string}> {
  ...
  if (conditions.includes("pcos")) {
    // Removed gender==="female" filter. Males with PCOS also have insulin
...
export interface PersonalizationProfile {
  ...
  dnaConsent: boolean;
  wakeUpTime?: WellnessUserProfile["wakeUpTime"];
}
...
export interface PDFGenerationOptions {
  ...
  addOns?: string[];
  /**
   * Phase 3: structured per-add-on scope (Family bundle member count,
   * Lab-Test cadence, free-form notes). Forwarded from the checkout
   * configuration so the renderer can reflect purchased quantities
   * (e.g. 2 vs 4 family reports) instead of relying on SKU-id fallbacks.
   */
...
function renderTopActions(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  if (hints.length >= 3) {
    ...
    ctx.yPosition += 6;
    addText(ctx, l(
      `You reported ${profile.inputs?.sleepHoursLabel ?? `${profile.sleepHours ?? 6} hrs/night`} — ${profile.sleepScore < 50 ? "this is well below the 7–8 hr range and worth fixing first" : profile.sleepScore < 70 ? "this is below the 7–8 hr range and worth tightening up" : "stay consistent to keep this on track"}. Consistent wake times improve sleep quality more than sleeping longer (Journal of Clinical Sleep Medicine, 2023).`,
      `आपने बताया ${profile.inputs?.sleepHoursLabel ?? `${profile.sleepHours ?? 6} घंटे/रात`} — ${profile.sleepScore < 50 ? "यह 7-8 घंटे की सीमा से बहुत कम है" : profile.sleepScore < 70 ? "यह 7-8 घंटे से कम है" : "इसे बनाए रखें"}। निरंतर जागने का समय अधिक सोने से बेहतर नींद देता है।`
    ), 9);
    addSpacing(ctx, 3);
    ...
    addText(ctx, l(
      `${exerciseSuggestion} Even 20 minutes of moderate activity reduces all-cause mortality by 30% (JAMA Internal Medicine, 2022). You reported your activity level as ${profile.activityLevel ?? "lightly active"}.`,
      `${exerciseSuggestion} 20 मिनट की मध्यम गतिविधि भी सभी कारणों से मृत्यु दर को 30% कम करती है। आपने अपनी गतिविधि स्तर ${profile.activityLevel ?? "लाइट"} बताई है।`
    ), 9);
  }
...
function renderWellnessBaseline(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  scores.forEach(({ icon, label, score, note }) => {
    ...
    setCtxFont(ctx, "bold");
    ctx.pdf.setFontSize(8);
    ctx.pdf.text(scoreLabel, 163, ctx.yPosition + 6, { align: "center" });
  ...
  if (lowScores.length > 0) {
    ...
    ctx.pdf.setFontSize(8);
    ctx.pdf.text(l(
      `${lowestScore.label} is the area furthest from a healthy range based on what you reported. Improving this first tends to lift the others, since these areas reinforce each other.`,
      `आपके बताए अनुसार ${lowestScore.label} सबसे अधिक सुधार चाहता है। इसे पहले ठीक करें — बाकी क्षेत्र भी आसान हो जाएंगे।`
...
function renderHealthScoreDashboard(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx,
    l("01 · Personal Health Snapshot", "01 · व्यक्तिगत स्वास्थ्य स्नैपशॉट"),
    l("Based on your quiz answers. Each row shows what you reported and how it compares to a healthy range — not a precision-measured score. Red = needs attention. Orange = below target. Green = on track.",
      "आपके क्विज़ उत्तरों के आधार पर। प्रत्येक पंक्ति आपके बताए मान और स्वस्थ सीमा से तुलना दिखाती है — मापा गया स्कोर नहीं।"));
  ...
  // Phase 4 — Honesty Pass: do NOT show a fabricated "X/100" overall
  // score. Instead, count how many dimensions need attention based on
  // the user's actual inputs and derived scores. This is honest about
  // what we actually know from the quiz.
  addSpacing(ctx, 3);
  checkPageBreak(ctx, 20);
  const dimsForCount = [sleepScore, stressMgmtScore, mealTimingScore, hydrationScore, gutScore, skinScore];
  const needsAttention = dimsForCount.filter(s => s < 60).length;
  const onTrack = dimsForCount.filter(s => s >= 70).length;
  const summaryX = ctx.margin;
  ctx.pdf.setFontSize(11);
  ctx.pdf.setTextColor(...NAVY);
  setCtxFont(ctx, "bold");
  ctx.pdf.text(
    l(`${needsAttention} of 6 areas need attention · ${onTrack} on track`,
      `6 में से ${needsAttention} क्षेत्रों पर ध्यान दें · ${onTrack} ठीक हैं`),
    summaryX, ctx.yPosition + 6);
  ctx.pdf.setFontSize(8);
  ctx.pdf.setTextColor(...GRAY);
  setCtxFont(ctx, "normal");
  ctx.pdf.text(
    l("Based on what you reported in the quiz — not measured biomarkers.",
      "आपके क्विज़ उत्तरों के आधार पर — मापा गया बायोमार्कर नहीं।"),
    summaryX, ctx.yPosition + 12);
  ctx.yPosition += 18;
  addSpacing(ctx, 2);
  ...
  // Phase 4 — Honesty Pass: prefer the user's actual input label
  // ("4–6 glasses/day (~1.0–1.5 L)") over the synthesised midpoint
  // decimal so the report doesn't pretend a categorical answer was a
  // measurement. Falls back to the legacy decimal phrasing only when
  // no input label is available (legacy stored profiles).
  const hydInputLabel = profile.inputs?.hydrationLabel;
  const hydLevel = profile.inputs?.hydrationLevel;
  const hydRootCause = hydInputLabel
    ? (hydLevel === "low"          ? l(`${hydInputLabel} — well below target ~${waterTarget} L`, `${hydInputLabel} — लक्ष्य ~${waterTarget} L से बहुत कम`)
      : hydLevel === "below-target" ? l(`${hydInputLabel} — below target ~${waterTarget} L`, `${hydInputLabel} — लक्ष्य ~${waterTarget} L से कम`)
      : hydLevel === "on-track"     ? l(`${hydInputLabel} — close to target ~${waterTarget} L`, `${hydInputLabel} — लक्ष्य ~${waterTarget} L के करीब`)
      :                                l(`${hydInputLabel}`, `${hydInputLabel}`))
    : waterLitersResolved < 1.0
    ? l(`Below ~1 L/day — severely low (target ~${waterTarget} L)`, `~1 L/दिन से कम — बहुत कम`)
    : waterLitersResolved < 1.5
    ? l(`Below target — aim for ~${waterTarget} L/day`, `लक्ष्य से कम — ~${waterTarget} L/दिन का लक्ष्य रखें`)
    : l(`Close to target ~${waterTarget} L/day`, `लक्ष्य ~${waterTarget} L/दिन के करीब`);
  ...
  ctx.pdf.text(l("STATUS", "स्थिति"), ctx.margin + labelColW + 2, ctx.yPosition);
  ctx.pdf.text(l("WHERE YOU ARE", "आप कहाँ हैं"), ctx.margin + labelColW + scoreColW + 2, ctx.yPosition);
  ctx.pdf.text(l("BASED ON YOUR ANSWER", "आपके उत्तर के आधार पर"), ctx.margin + labelColW + scoreColW + barColW + 4, ctx.yPosition);
  ctx.yPosition += 5;
  ...
  dims.forEach((d, i) => {
    ...
    // Phase 4 — Honesty Pass: replace the "X/100" number with a 3-bucket
    // category label (Needs work / Below target / On track) so the report
    // doesn't claim numeric precision where the underlying input was a
    // category. The progress bar becomes a 3-step indicator (1/3, 2/3,
    // 3/3 filled) for the same reason — no decimal score is implied.
    const statusColor: [number, number, number] = d.score < 40 ? RED_ALERT : d.score < 70 ? ORANGE_WARN : GREEN_OK;
    const bucketLabel = d.score < 40 ? l("Needs work", "ध्यान दें")
      : d.score < 70 ? l("Below target", "लक्ष्य से कम")
      : l("On track", "ठीक है");
    ctx.pdf.setFontSize(7.5);
    ctx.pdf.setTextColor(...statusColor);
    setCtxFont(ctx, "bold");
    ctx.pdf.text(bucketLabel, ctx.margin + labelColW + 2, ctx.yPosition + 7);

    // 3-step indicator (no decimal score)
    const barX = ctx.margin + labelColW + scoreColW + 2;
    const barY = ctx.yPosition + 3;
    const barH = 5;
    const stepGap = 2;
    const stepW = (barColW - stepGap * 2) / 3;
    const filledSteps = d.score < 40 ? 1 : d.score < 70 ? 2 : 3;
    for (let s = 0; s < 3; s++) {
      const stepX = barX + s * (stepW + stepGap);
      if (s < filledSteps) {
        ctx.pdf.setFillColor(...statusColor);
      } else {
        ctx.pdf.setFillColor(220, 224, 230);
      }
      ctx.pdf.rect(stepX, barY, stepW, barH, "F");
    }
  ...
  ctx.pdf.setTextColor(...NAVY);
  setCtxFont(ctx, "bold");
  ctx.pdf.text(l(`WHY ${worst.name.toUpperCase()} NEEDS YOUR ATTENTION FIRST`, `${worst.name.toUpperCase()} पर पहले ध्यान क्यों`), ctx.margin + 4, ctx.yPosition + 7);
  ctx.pdf.setFontSize(8);
  ctx.pdf.setTextColor(...DARK);
  setCtxFont(ctx, "normal");
  // Phase 4 — Honesty Pass: drop the "Score: X/100" framing. Reference
  // the user's actual reported state instead, and avoid quantitative
  // promises ("5 of 6 scores improve in 3 weeks") we can't substantiate.
  const whyText = l(
    `Of the six areas you reported on, ${worst.name.toLowerCase()} is the one furthest from a healthy range — ${worst.root}. Improving this first tends to make the others easier to address, because sleep, hydration, meal timing and stress reinforce each other.`,
    `आपके बताए छह क्षेत्रों में, ${worst.name} सबसे अधिक ध्यान चाहता है — ${worst.root}। इसे पहले सुधारने से बाकी क्षेत्रों में भी सुधार आसान हो जाता है।`
  );
...
function renderMetabolicCalculations(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  drawPremiumTable(ctx,
    ...
    [
      // Filter food lists by intolerances.
      ...
      [l("Carbohydrates", "कार्बोहाइड्रेट"), `${_macroCarbsG}g`, `${carbsPct}%`,
        filterFoodLine(l("Oats, brown rice, millets, sweet potato, banana", "ओट्स, ब्राउन राइस, बाजरा, शकरकंद, केला"), profile.foodIntolerances)],
      [l("Healthy Fats", "स्वास्थ्य वसा"), `${_macroFatsG}g`, `${fatsPct}%`,
        filterFoodLine(l("Ghee, peanut butter, walnuts, flaxseeds, olive oil", "घी, पीनट बटर, अखरोट, अलसी, जैतून तेल"), profile.foodIntolerances)],
    ],
    [35, 28, 25, 102]
  );
  ...
  const adaptiveRows = wantsGain ? [
    ...
    [l("Energy crash or weakness", "ऊर्जा गिरावट या कमजोरी"), filterFoodLine(l("Add 1 banana + 1 tbsp peanut butter pre-workout", "प्री-वर्कआउट केला + PB"), profile.foodIntolerances), l("Immediate", "तुरंत")],
...
function renderDecisionEngine(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const rules: string[][] = [];
  // only emit a "fix wake time" recommendation when both
  // (a) the user's sleep score is poor AND
  // (b) the user is NOT already waking in/before the target window.
  // Users who reported "before-6" or "6-8" already wake early enough — telling
  // them to "fix" their wake time is contradictory and was a documented bug.
  const _wakeSlot = profile.wakeUpTime || "6-8";
  const _alreadyEarly = _wakeSlot === "before-6" || _wakeSlot === "6-8";
  if ((profile.sleepScore || 50) < 70 && !_alreadyEarly) {
    const { wakeTime: _wt, bedTime: _bt } = getSleepWindow(_wakeSlot);
    rules.push([l(`IF sleep < 6 hrs (yours = ${sleepHours})`, `IF नींद < 6 घंटे (आपकी = ${sleepHours})`),
                l(`Shift wake time earlier (target ${_wt}); bed by ${_bt}`, `जागने का समय जल्दी करें (लक्ष्य ${_wt}); सोना ${_bt}`),
                l("Cortisol/thyroid cascade", "कोर्टिसोल/थायरॉइड")]);
  } else if ((profile.sleepScore || 50) < 70 && _alreadyEarly) {
    // Sleep is poor but wake-time is fine — focus on sleep quality, not the schedule.
    rules.push([l(`IF sleep < 6 hrs (yours = ${sleepHours})`, `IF नींद < 6 घंटे (आपकी = ${sleepHours})`),
                l("Hold your current wake time; tighten wind-down routine + bed by 11pm", "अपना वर्तमान जागने का समय बनाए रखें; सोने की दिनचर्या कसें + 11 बजे तक बिस्तर"),
                l("Cortisol/thyroid cascade", "कोर्टिसोल/थायरॉइड")]);
  }
  // Phase 4 — Honesty Pass: cite the user's actual hydration answer
  // (e.g. "4–6 glasses/day") instead of the synthesised midpoint
  // decimal. The decimal made the table look measured when it was
  // categorical.
  if (waterLiters < 1.5) {
    const _hydAnswer = profile.inputs?.hydrationLabel ?? `${waterLiters}L`;
    rules.push([l(`IF water below ~1.5 L/day (you reported: ${_hydAnswer})`, `IF पानी < ~1.5 L/दिन (आपने बताया: ${_hydAnswer})`),
  ...
  if (mealFreq < 3)
    rules.push([l(`IF meals < 3/day (yours = ${mealFreq})`, `IF भोजन < 3/दिन (आपका = ${mealFreq})`),
                ...
                // Removed the speculative "~${mealFreq * 700} max" calorie cap —
                // it was inventing a per-meal ceiling we cannot defend.
                l(`Calorie target ${tdee + 250}+ kcal/day is hard to hit on ${mealFreq} meals`, "कैलोरी लक्ष्य पूरा करना मुश्किल")]);
  ...
  if (conds.some((c: string) => /lactose/i.test(c)))
    rules.push([l("IF lactose intolerant (yes)", "IF लैक्टोज असहिष्णु (हाँ)"),
                l(
                  `All dairy → ${filterFoodLine("lactose-free milk, coconut curd, almond milk, oat milk", profile.foodIntolerances)}`,
                  `सभी डेयरी → ${filterFoodLine("लैक्टोज-फ्री दूध, नारियल दही, बादाम दूध, ओट दूध", profile.foodIntolerances)}`,
                ),
  ...
  // Use same score formulas as Dashboard — was using stale mealFreq/3 and waterVal default 1.2
  ...
  // Phase 4 — Honesty Pass: replace the "X/100" column and the
  // false-precision "+25-35 pts in 3 weeks" promise with status
  // categories and qualitative direction. The ordering still surfaces
  // what to address first, without inventing numbers.
  const directionLabel = (score: number): string =>
    score < 40 ? l("Big improvement available", "बड़ा सुधार संभव") :
    score < 60 ? l("Meaningful improvement available", "महत्वपूर्ण सुधार संभव") :
                  l("Small refinement", "थोड़ा सुधार");
  drawPremiumTable(ctx,
    [l("Priority", "प्राथमिकता"), l("Factor", "कारक"), l("Status", "स्थिति"), l("Impact on Goal", "लक्ष्य पर प्रभाव"), l("Direction", "दिशा")],
    dimScores.map((d, i) => [
      `#${i + 1} — ${priorLabel(d.score)}`,
      d.name,
      d.score < 40 ? l("Needs work", "ध्यान दें") : d.score < 70 ? l("Below target", "लक्ष्य से कम") : l("On track", "ठीक है"),
      impactDesc(d.name, d.score, i),
      directionLabel(d.score),
    ]),
    [32, 30, 28, 64, 36]
  );
...
function renderWeightProjection(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx,
    l("04 · 90-Day Weight Roadmap (Estimate)", "04 · 90-दिन का वजन रोडमैप (अनुमान)"),
    headerNote + l(" · Estimates assume consistent adherence; actual results vary.", " · अनुमान निरंतर पालन मानते हैं; वास्तविक परिणाम भिन्न हो सकते हैं।"));
  ...
  const macros = [
    {
      ...
      sources: isVeg
        ...
        ? filterFoodLine(l("Sprouts, lentils, paneer, tofu, Greek yogurt, soya", "अंकुरित अनाज, दाल, पनीर, टोफू, ग्रीक दही, सोया"), profile.foodIntolerances)
        : filterFoodLine(l("Eggs, chicken, fish, paneer, lentils, Greek yogurt", "अंडे, चिकन, मछली, पनीर, दाल, ग्रीक दही"), profile.foodIntolerances),
    },
    {
      name: l("CARBOHYDRATES", "कार्बोहाइड्रेट"),
      grams: profile.carbsGrams,
      pct: carbsPct,
      // also pass carbs/fats source lists through the intolerance hard-filter.
      sources: filterFoodLine(l("Oats, millets, brown rice, sweet potato, banana", "ओट्स, बाजरा, भूरे चावल, शकरकंद, केला"), profile.foodIntolerances),
    },
    {
      name: l("HEALTHY FATS", "स्वस्थ वसा"),
      grams: profile.fatsGrams,
      pct: fatsPct,
      sources: filterFoodLine(l("Ghee, olive oil, walnuts, peanut butter, flaxseeds", "घी, जैतून का तेल, अखरोट, मूंगफली का मक्खन, अलसी"), profile.foodIntolerances),
    },
  ];
...
function renderNutritionStrategy(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  // Phase 2 — variant-driven principles paragraph + bullets from content-blocks.
  const _nutritionDecision = selectVariants(profile);
  const _nutritionVariant = CONTENT_BLOCKS.nutritionPrinciples[_nutritionDecision.variants.nutritionPrinciples];
  if (_nutritionVariant) {
    _nutritionVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
    if (_nutritionVariant.bullets) {
      _nutritionVariant.bullets.forEach(b => addBullet(ctx, language === "hi" ? b.hi : b.en, 8));
    }
    addSpacing(ctx, 2);
  }
  addText(ctx, l("Build every meal with these four components:", "हर भोजन इन चार घटकों से बनाएं:"), 9);
  // Filter food bullets by intolerances.
  // before rendering. Previously a seafood-intolerant non-veg user still got
  // "fish (salmon, mackerel)" and "fatty fish" recommended on the page.
  const fi = profile.foodIntolerances;
  if (isVegan) {
    addBullet(ctx, filterFoodLine(l("Protein: Tofu, tempeh, legumes (dal, chickpeas, rajma), seitan, quinoa", "प्रोटीन: टोफू, टेम्पेह, दालें (चना, राजमा), सीटान, क्विनोआ"), fi), 8);
    addBullet(ctx, filterFoodLine(l("Complex Carbs: Brown rice, millets (ragi, jowar), sweet potato, oats, quinoa", "जटिल कार्ब्स: ब्राउन राइस, बाजरा (रागी, ज्वार), शकरकंद, ओट्स, क्विनोआ"), fi), 8);
    ...
    addBullet(ctx, filterFoodLine(l("Healthy Fats: Coconut oil, olive oil, flaxseeds, chia seeds, walnuts, avocado", "स्वस्थ वसा: नारियल तेल, जैतून का तेल, अलसी, चिया बीज, अखरोट, एवोकाडो"), fi), 8);
  } else if (isVeg) {
    addBullet(ctx, filterFoodLine(l("Protein: Paneer, Greek yogurt, eggs, dal (moong/arhar/masoor), chickpeas, tofu", "प्रोटीन: पनीर, ग्रीक दही, अंडे, दाल (मूंग/अरहर/मसूर), चना, टोफू"), fi), 8);
    addBullet(ctx, filterFoodLine(l("Complex Carbs: Brown rice, whole wheat roti, oats, millets, sweet potato", "जटिल कार्ब्स: ब्राउन राइस, पूरी गेहूं की रोटी, ओट्स, बाजरा, शकरकंद"), fi), 8);
    ...
    addBullet(ctx, filterFoodLine(l("Healthy Fats: Ghee, olive oil, nuts (almonds, walnuts), seeds, coconut", "स्वस्थ वसा: घी, जैतून का तेल, मेवे (बादाम, अखरोट), बीज, नारियल"), fi), 8);
  } else {
    addBullet(ctx, filterFoodLine(l("Protein: Chicken breast, fish (salmon, mackerel), eggs, paneer, dal, legumes", "प्रोटीन: चिकन ब्रेस्ट, मछली (सालमन, मैकेरल), अंडे, पनीर, दाल, फलियां"), fi), 8);
    addBullet(ctx, filterFoodLine(l("Complex Carbs: Brown rice, whole wheat roti, oats, millets, sweet potato", "जटिल कार्ब्स: ब्राउन राइस, पूरी गेहूं की रोटी, ओट्स, बाजरा, शकरकंद"), fi), 8);
    addBullet(ctx, l("Vegetables: Minimum 3 servings/day — spinach, broccoli, bell peppers, carrots, gourds", "सब्जियां: न्यूनतम 3 सर्विंग/दिन — पालक, ब्रोकली, शिमला मिर्च, गाजर, लौकी"), 8);
    addBullet(ctx, filterFoodLine(l("Healthy Fats: Ghee, olive oil, nuts (almonds, walnuts), fatty fish, seeds", "स्वस्थ वसा: घी, जैतून का तेल, मेवे (बादाम, अखरोट), वसायुक्त मछली, बीज"), fi), 8);
  }
  ...
  if (profile.foodIntolerances?.length > 0) {
    ...
    // Substitution bullets — each candidate substitute list is run through
    // filterFoodLine against the user's FULL intolerance set so a multi-
    // intolerance user (e.g. lactose + nuts) never sees a contraindicated
    // substitute (e.g. "almond milk").
    const fi = profile.foodIntolerances;
    profile.foodIntolerances.forEach(intolerance => {
      ...
      if (lower.includes("lactose") || lower.includes("dairy")) {
        const en = filterFoodLine("almond milk, oat milk, coconut yogurt, tofu", fi);
        const hi = filterFoodLine("बादाम दूध, ओट दूध, नारियल दही, टोफू", fi);
        addBullet(ctx, l(`Dairy-free: Use ${en} instead of dairy`, `डेयरी-मुक्त: डेयरी के बजाय ${hi} उपयोग करें`), 8);
        ...
        const en = filterFoodLine("sunflower seeds, pumpkin seeds, coconut, soy", fi);
        const hi = filterFoodLine("सूरजमुखी बीज, कद्दू बीज, नारियल, सोया", fi);
        addBullet(ctx, l(`Nut-free: Use ${en} for healthy fats`, `मेवा-मुक्त: स्वस्थ वसा के लिए ${hi} उपयोग करें`), 8);
      } else if (lower.includes("soy")) {
        const en = filterFoodLine("paneer, chickpea flour, hemp protein", fi);
        const hi = filterFoodLine("पनीर, चने का आटा, हेम्प प्रोटीन", fi);
        addBullet(ctx, l(`Soy-free: Use ${en} as alternatives`, `सोया-मुक्त: विकल्प के रूप में ${hi}`), 8);
      } else {
...
function renderSleepProtocol(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, l("07 · Sleep & Energy Protocol", "07 · नींद और ऊर्जा प्रोटोकॉल"), l(`You reported: ${profile.inputs?.sleepHoursLabel ?? `${profile.sleepHours ?? 6} hrs/night`} · Target: 7–8 hrs nightly`, `आपने बताया: ${profile.inputs?.sleepHoursLabel ?? `${profile.sleepHours ?? 6} घंटे/रात`} · लक्ष्य: 7-8 घंटे रात`));
  ...
  // Phase 2 — wind-down framing from content-blocks (severe / moderate / healthy).
  const _sleepDecision = selectVariants(profile);
  const _sleepVariant = CONTENT_BLOCKS.sleepProtocol[_sleepDecision.variants.sleepProtocol];
  if (_sleepVariant) {
    _sleepVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
    if (_sleepVariant.bullets) {
      _sleepVariant.bullets.forEach(b => addBullet(ctx, language === "hi" ? b.hi : b.en, 8));
    }
    addSpacing(ctx, 2);
  }
...
function renderMovementProgram(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, title, l(`${profile.name}'s personalized exercise protocol — You reported activity level: ${profile.activityLevel ?? "lightly active"}`, `${profile.name} का व्यक्तिगत व्यायाम प्रोटोकॉल — आपने बताई गतिविधि: ${profile.activityLevel ?? "लाइट"}`));
  ...
  if (tier === "essential") {
    ...
    addText(ctx, l(`Designed for sustainable habit building at your current activity level (${profile.activityLevel ?? "lightly active"}). Each session is 25-35 minutes.`, `आपके वर्तमान गतिविधि स्तर (${profile.activityLevel ?? "lightly active"}) पर टिकाऊ आदत निर्माण के लिए। प्रत्येक सत्र 25-35 मिनट है।`), 9);
    addSpacing(ctx, 2);
...
function renderStressManagement(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, l("08 · Anxiety & Mood Management", "08 · चिंता और मनोदशा प्रबंधन"), l(`You reported stress as: ${profile.inputs?.stressLevelLabel ?? "moderate (self-reported)"} · ${_stressMgmtScore < 45 ? "Needs attention — daily tools recommended" : _stressMgmtScore < 70 ? "Below target — daily tools recommended" : "Well managed — keep current practices"}`, `आपने बताया तनाव: ${profile.inputs?.stressLevelLabel ?? "मध्यम"} · ${_stressMgmtScore < 45 ? "ध्यान दें — दैनिक उपकरण" : _stressMgmtScore < 70 ? "लक्ष्य से कम — दैनिक उपकरण" : "अच्छा — अभ्यास बनाए रखें"}`));
  ...
  // Phase 2 — variant-driven framing paragraph + bullets.
  const _stressDecision = selectVariants(profile);
  const _stressVariant = CONTENT_BLOCKS.stressProtocol[_stressDecision.variants.stressProtocol];
  if (_stressVariant) {
    _stressVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
    if (_stressVariant.bullets) {
      _stressVariant.bullets.forEach(b => addBullet(ctx, language === "hi" ? b.hi : b.en, 8));
    }
    addSpacing(ctx, 2);
  }
  ...
  if (tier !== "free") {
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        [l("Evening", "शाम"), l("15-20 min tech-free. Write 3 gratitude items", "15-20 मिनट टेक-फ्री। 3 कृतज्ञता लिखें"), l("Research supports anxiety reduction with consistent practice", "निरंतर अभ्यास से चिंता में कमी का शोध समर्थन")],
...
function renderSupplementStrategy(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  // Phase 2 — variant-driven framing for the section intro.
  const decisions = selectVariants(profile);
  const introVariant = CONTENT_BLOCKS.supplementStrategy[decisions.variants.supplementStrategy];
  if (introVariant) {
    introVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
  }
  addText(ctx, l(
    "Add no more than one new supplement every 14 days. Each row below shows the evidence tier — T1 = strong RCT support, T2 = mechanistic + smaller trials, T3 = traditional / emerging.",
    "हर 14 दिन में एक से अधिक नया सप्लीमेंट न जोड़ें। हर पंक्ति में साक्ष्य स्तर दिखाया है — T1 = मजबूत RCT, T2 = मैकेनिज़्म + छोटे ट्रायल, T3 = पारंपरिक/उभरते।"
  ), 9);
  addSpacing(ctx, 3);

  addSubSection(ctx, l("Personalised Supplement Stack", "व्यक्तिगत सप्लीमेंट स्टैक"));

  // Build rows from the catalog-driven decision result.
  const phaseLabel = (cat: string): string => {
    switch (cat) {
      case "foundation":          return l("Phase 1\nWeeks 1-4\nFoundation", "चरण 1\nसप्ताह 1-4\nआधार");
      case "condition-specific":  return l("Phase 1\nWeeks 1-4\nCondition", "चरण 1\nसप्ताह 1-4\nस्थिति");
      case "targeted":            return l("Phase 2\nWeeks 5-8\nTargeted",   "चरण 2\nसप्ताह 5-8\nलक्षित");
      case "maintenance":         return l("Phase 3\nWeeks 9-12\nMaintenance", "चरण 3\nसप्ताह 9-12\nरखरखाव");
      default:                    return l("Phase 1", "चरण 1");
    }
  };

  // Order so that condition-specific surfaces in Phase 1, then foundation, targeted, maintenance.
  const orderRank: Record<string, number> = { "condition-specific": 0, foundation: 1, targeted: 2, maintenance: 3 };
  const sortedDecisions = [...decisions.supplementResult.included]
    .sort((a, b) => (orderRank[a.supplement.category] ?? 9) - (orderRank[b.supplement.category] ?? 9));

  let prevPhaseLabel = "";
  const suppRows: string[][] = sortedDecisions.map(dec => {
    const supp = dec.supplement;
    const name = language === "hi" && supp.nameHi ? supp.nameHi : supp.name;
    const dose = language === "hi" && supp.doseHi ? supp.doseHi : supp.dose;
    const timing = language === "hi" && supp.timingHi ? supp.timingHi : supp.timing;
    const evidence = language === "hi" && supp.evidenceSummaryHi ? supp.evidenceSummaryHi : supp.evidenceSummary;
    const cautionPrefix =
      dec.action === "doctor-first" ? "★WARN★ " :
      dec.action === "swap"         ? "★WARN★ " :
      "";
    const noteText = language === "hi" && dec.reasonHi ? dec.reasonHi : dec.reason;
    const note = noteText ? ` ${noteText}` : "";
    const phaseCell = phaseLabel(supp.category);
    const phaseDisplay = phaseCell === prevPhaseLabel ? "" : phaseCell;
    prevPhaseLabel = phaseCell;
    return [
      phaseDisplay,
      `${name}\n${tierBadge(supp.evidenceTier)}`,
      `${dose}\n${timing}`,
      `${cautionPrefix}${evidence}${note}`,
      supp.approxCostInr,
    ];
  });

  drawPremiumTable(ctx,
    [l("Phase", "चरण"), l("Supplement (Tier)", "सप्लीमेंट (स्तर)"), l("Dose + Timing", "खुराक + समय"), l("Why for You", "आपके लिए क्यों"), l("Cost/Month", "लागत/माह")],
    suppRows,
    [26, 36, 48, 54, 16]
  );

  // Rejection log — show which supplements were excluded by contraindications.
  if (decisions.supplementResult.rejected.length > 0) {
    addSpacing(ctx, 3);
    addSubSection(ctx, l("Excluded For Your Profile", "आपकी प्रोफ़ाइल के लिए वर्जित"));
    decisions.supplementResult.rejected.forEach(r => {
      addBullet(ctx, l(
        `${r.supplementName} — ${r.reason}`,
        `${r.supplementName} — ${r.reasonHi || r.reason}`,
      ));
    });
  }
  ...
  if (profile.sleepScore < 65 || /^(veg|vegan|eggetarian)/i.test(profile.dietaryPreference || "")) {
    highTests.push({
      ...
      when: l("Week 0", "सप्ताह 0"),
        why: (() => {
        const isVeg = /^(veg|vegan|eggetarian)/i.test(profile.dietaryPreference || "");
        // B12 reason was hardcoded as "Vegetarian diet" for ALL users.
...
function renderDigestiveHealth(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  // Phase 2 — symptom-routed gut variant body + bullets.
  const _gutDecision = selectVariants(profile);
  const _gutVariant = CONTENT_BLOCKS.gutHealth[_gutDecision.variants.gutHealth];
  if (_gutVariant) {
    _gutVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
    if (_gutVariant.bullets) {
      _gutVariant.bullets.forEach(b => addBullet(ctx, language === "hi" ? b.hi : b.en, 8));
    }
    addSpacing(ctx, 2);
  }
...
function renderSkinHealth(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  // Phase 2 — concern-routed skin variant body + bullets.
  const _skinDecision = selectVariants(profile);
  const _skinVariant = CONTENT_BLOCKS.skinHealth[_skinDecision.variants.skinHealth];
  if (_skinVariant) {
    _skinVariant.body.forEach(p => addText(ctx, language === "hi" ? p.hi : p.en, 9));
    if (_skinVariant.bullets) {
      _skinVariant.bullets.forEach(b => addBullet(ctx, language === "hi" ? b.hi : b.en, 8));
    }
    addSpacing(ctx, 2);
  }
  ...
  drawPremiumTable(ctx,
    ...
    [
      ...
      // Filter omega-3 source list by intolerances.
      [l("Omega-3", "ओमेगा-3"), filterFoodLine(l("Flaxseeds, walnuts, fatty fish, algae oil", "अलसी, अखरोट, मछली, शैवाल तेल"), profile.foodIntolerances), l("Barrier repair, inflammation control", "त्वचा बाधा, सूजन नियंत्रण"), l("2g EPA+DHA", "2g EPA+DHA")],
      ...
      [l("Vitamin A", "विटामिन A"), filterFoodLine(l("Sweet potato, carrots, spinach, eggs", "शकरकंद, गाजर, पालक, अंडे"), profile.foodIntolerances), l("Cell turnover, anti-aging", "कोशिका नवीनीकरण, उम्र-रोधी"), l("700-900mcg", "700-900mcg")],
  ...
  profile.skinConcerns.forEach(concern => {
    ...
    if (lower.includes("acne")) {
      ...
      {
        // Filter both language branches separately so Hindi PDFs do not surface
        // contraindicated foods either.
        const fi = profile.foodIntolerances;
        const enFats   = filterFoodLine("flaxseed oil, walnuts", fi);
        const enVitE   = filterFoodLine("almonds, sunflower seeds", fi);
        const enOils   = filterFoodLine("coconut oil, almond oil", fi);
        const hiFats   = filterFoodLine("अलसी तेल, अखरोट", fi);
        const hiVitE   = filterFoodLine("बादाम, सूरजमुखी बीज", fi);
        const hiOils   = filterFoodLine("नारियल तेल, बादाम तेल", fi);
        drawGoldCallout(ctx, l("Dry Skin Action Plan", "शुष्क त्वचा कार्य योजना"), l(
          `1. Omega-3 daily (${enFats}). 2. Vitamin E: ${enVitE}. 3. Apply ${enOils} WHILE skin is still damp (within 3 min of bathing). 4. Avoid hot showers — they strip natural oils.`,
          `1. ओमेगा-3 रोज (${hiFats})। 2. विटामिन E: ${hiVitE}। 3. नहाने के बाद गीली त्वचा पर ${hiOils} लगाएं। 4. गर्म पानी से नहाने से बचें।`
        ));
      }
...
/**
 * Phase 3 — render a content-block intro for an add-on section.
 * Falls back to the supplied legacy text if the variant is missing.
 */
function renderAddonIntroBlock(
  ctx: PDFContext,
  section: SectionId,
  variantId: string,
  language: "en" | "hi",
  fallback: { body: string; bullets?: string[] },
): void {
  const variant = getVariant(section, variantId);
  if (!variant) {
    addText(ctx, fallback.body, 8.5, DARK);
    if (fallback.bullets) {
      fallback.bullets.forEach(b => addText(ctx, `• ${b}`, 8.5, DARK));
    }
    addSpacing(ctx, 4);
    return;
  }
  for (const para of variant.body) {
    addText(ctx, language === "hi" ? para.hi : para.en, 8.5, DARK);
    addSpacing(ctx, 1);
  }
  if (variant.bullets) {
    for (const bullet of variant.bullets) {
      addText(ctx, `• ${language === "hi" ? bullet.hi : bullet.en}`, 8.5, DARK);
    }
  }
  addSpacing(ctx, 4);
}
...
function renderAddOns(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  const { profile } = bundle;
  const selected = bundle.addOns || [];

  // Phase 3 — resolve effective add-ons (selected ∪ tier-bundled ∪ auto-included).
  // Map PDF tier ↔ plan_id for tier-bundling. Widened to plain string keys
  // so values like "subscription" (which the bundle carries at runtime
  // even though the static type is narrower) resolve correctly instead
  // of silently falling back to essential.
  ...
  const planId = tierToPlan[bundle.tier as string] ?? "essential_blueprint";
  const effective = computeEffectiveAddOns(selected, profile, planId);
  const addOns = effective.ids;
  const decisions = selectVariants(profile, addOns);
  ...
  if (addOns.includes("addon_dna")) {
    ...
    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonDnaIntro", decisions.variants.addonDnaIntro, language, {
      body: l("What follows is NOT theoretical genetics — it is an IF/THEN decision engine derived from the top 3 genes that affect YOUR profile: your sleep score, stress level, and training preferences. Each gene links directly to a dietary or training decision.",
             "यह सैद्धांतिक आनुवांशिकी नहीं है — यह एक IF/THEN निर्णय इंजन है जो उन शीर्ष 3 जीन से बना है जो आपके प्रोफ़ाइल को प्रभावित करते हैं।"),
    });
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        [
          ...
          profile.sleepScore < 65
            ? l(`CRITICAL for you (you reported ${profile.inputs?.sleepHoursLabel ?? "disrupted sleep"}): Cut all caffeine after 12pm. Switch to green tea before workouts.`,
                `आपके लिए गंभीर (आपने ${profile.inputs?.sleepHoursLabel ?? "कम नींद"} बताई): दोपहर 12 बजे बाद सभी कैफीन बंद करें। वर्कआउट से पहले ग्रीन टी लें।`)
            : l(`Limit to 2 cups before noon. You reported ${profile.inputs?.sleepHoursLabel ?? "borderline sleep"} — protection matters.`,
                `दोपहर से पहले 2 कप तक सीमित करें। आपने ${profile.inputs?.sleepHoursLabel ?? "सीमांत नींद"} बताई — सुरक्षा जरूरी।`),
    ...
    drawGoldCallout(ctx,
      ...
      profile.sleepScore < 65
        ? l(`CYP1A2 × Sleep: You reported ${profile.inputs?.sleepHoursLabel ?? "disrupted sleep"}. Slow caffeine metabolism + poor sleep = cortisol elevation. Cut all caffeine after 12pm starting TODAY. This single change improves sleep, stress, and energy simultaneously.`,
            `CYP1A2 × नींद: आपने ${profile.inputs?.sleepHoursLabel ?? "कम नींद"} बताई। धीमी कैफीन चयापचय + खराब नींद = कोर्टिसोल वृद्धि। आज से दोपहर 12 बजे बाद सभी कैफीन बंद करें।`)
  ...
  if (addOns.includes("addon_supplement")) {
    ...
    drawSectionBanner(ctx,
      ...
      l(`${profile.name} · Stress: ${stressLevel} · Sleep: ${profile.inputs?.sleepHoursLabel ?? "self-reported"} · BMI: ${bmi} — Personalized cycling & timing protocol`,
        `${profile.name} · तनाव: ${stressLevel} · नींद: ${profile.inputs?.sleepHoursLabel ?? "स्व-रिपोर्ट"} · BMI: ${bmi} — व्यक्तिगत साइकलिंग और टाइमिंग प्रोटोकॉल`)
    );

    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonSupplementIntro", decisions.variants.addonSupplementIntro, language, {
      body: l(`This protocol goes BEYOND the basic supplement list in your core plan. It tells you WHEN to cycle each supplement, HOW to stack them safely, and HOW to adjust based on your results. Designed for ${profile.name}: ${profile.age} yrs, BMI ${bmi}, stress level ${stressLevel}.`,
             `यह प्रोटोकॉल आपकी मूल योजना से परे है — साइकलिंग, स्टैकिंग और समायोजन बताता है।`),
    });
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        [l("Phase 1\nWeeks 1–4","चरण 1\nसप्ताह 1-4"),
          ...
          // Skip fish oil for vegetarians or seafood-intolerant.
          // users. The previous fallback string offered "fish oil or algae-based" which
          // was a direct contraindication for seafood-intolerant non-veg users.
          ((/^(veg|vegan|eggetarian)/i.test(profile.dietaryPreference || "")) ||
            (profile.foodIntolerances || []).some(f => /seafood|^fish$|^fish_/i.test(f.trim())))
            ? l("★WARN★ Use algae-based omega-3 (not fish). Critical for brain + cardiovascular.", "★WARN★ शैवाल-आधारित ओमेगा-3 का उपयोग करें (मछली नहीं)। मस्तिष्क + हृदय के लिए महत्वपूर्ण।")
        ...
        [l("Phase 2\nWeeks 5–8\nTargeted","चरण 2\nसप्ताह 5-8\nलक्षित"),
          ...
          (profile.stressScore || 50) > 55
            ? l(`★CRIT★ Your stress is ${stressLevel}. KSM-66 is clinically studied for cortisol support — consistent use over weeks is needed to assess your response.`,`★CRIT★ आपका तनाव ${stressLevel} है। KSM-66 कोर्टिसोल समर्थन के लिए नैदानिक रूप से अध्ययन किया गया है।`)
  ...
  if (addOns.includes("addon_athlete")) {
    ...
    drawSectionBanner(ctx,
      ...
      l(`${profile.name} · ${profile.weightKg}kg · Activity: ${profile.activityLevel ?? "active"} · Max HR ${maxHR} bpm — 12-Week structured athlete protocol`,
        `${profile.name} · ${profile.weightKg}kg · गतिविधि: ${profile.activityLevel ?? "सक्रिय"} · अधिकतम HR ${maxHR} bpm — 12-सप्ताह संरचित एथलीट प्रोटोकॉल`)
    );

    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonAthleteIntro", decisions.variants.addonAthleteIntro, language, {
      body: l(`This section is your competitive edge — it goes beyond the general movement protocol in your core plan. It gives you periodization science, sport-specific fueling, and heart-rate zone training computed from YOUR data (Max HR: ${maxHR} bpm, Weight: ${profile.weightKg}kg, TDEE: ${tdee} kcal).`,
             `यह अनुभाग आपका प्रतिस्पर्धात्मक लाभ है — आपके डेटा से गणना की गई आवधिकता और हृदय-दर क्षेत्र प्रशिक्षण।`),
    });
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        [l("Pre-sleep","सोने से पहले"),
          filterFoodLine(l("Casein protein, 100g paneer, Greek yogurt", "कैसिन प्रोटीन, 100g पनीर, ग्रीक दही"), profile.foodIntolerances),
  ...
  // ════════════════════════════════════════════════════════════
  if (addOns.includes("addon_family") || addOns.includes("addon_family_2") || addOns.includes("addon_family_4")) {
    addNewPage(ctx);
    drawSectionBanner(ctx,
    ...
    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonFamilyIntro", decisions.variants.addonFamilyIntro, language, {
      body: l(`Your personal plan is built for you. This add-on extends the same evidence-base to your entire family. Each age group has DIFFERENT nutritional priorities — this table tells you exactly what to adjust from your existing meals for each family member.`,
             `आपकी व्यक्तिगत योजना आपके लिए बनाई गई है। यह ऐड-ऑन उसी साक्ष्य-आधार को आपके पूरे परिवार तक विस्तारित करता है।`),
    });

    // ── Phase 3: per-member roster honoring purchased family scope.
    // Drives the bundle quantity (2 vs 4 reports) from `addOnScope`,
    // falling back to the SKU id, then to a 2-report default.
    const familyScope =
      bundle.addOnScope?.["addon_family_4"] ??
      bundle.addOnScope?.["addon_family_2"] ??
      bundle.addOnScope?.["addon_family"];
    const memberCount: 2 | 4 =
      (familyScope?.familyMemberCount as 2 | 4 | undefined) ??
      (addOns.includes("addon_family_4") ? 4 : 2);
    addSpacing(ctx, 2);
    addSubSection(
      ctx,
      l(
        `Your Family Bundle — ${memberCount} Personalised Reports`,
        `आपका पारिवारिक बंडल — ${memberCount} व्यक्तिगत रिपोर्ट`,
      ),
    );
    type MemberSlot = {
      title: { en: string; hi: string };
      ageBand: { en: string; hi: string };
      calories: string;
      protein: { en: string; hi: string };
      focus: { en: string; hi: string };
      avoid: { en: string; hi: string };
    };
    const adultCalLow = Math.max(1400, tdee - 200);
    const adultCalHigh = tdee + 200;
    const memberSlots: MemberSlot[] = memberCount === 4
      ? [
          {
            title: { en: `Report 1 of 4 — Primary adult: ${profile.name} (age ${profile.age})`,
                     hi: `4 में से रिपोर्ट 1 — प्रमुख वयस्क: ${profile.name} (आयु ${profile.age})` },
            ageBand: { en: "Adult 20–45", hi: "वयस्क 20-45" },
            calories: `${adultCalLow}–${adultCalHigh} kcal`,
            protein: { en: `${Math.round((profile.weightKg || 65) * 1.6)}–${Math.round((profile.weightKg || 65) * 2.0)} g/day`,
                       hi: `${Math.round((profile.weightKg || 65) * 1.6)}–${Math.round((profile.weightKg || 65) * 2.0)} g/दिन` },
            focus: { en: "B12, D3 2,000 IU, Magnesium, fibre 30 g/day", hi: "B12, D3 2,000 IU, मैग्नीशियम, फाइबर 30 g/दिन" },
            avoid: { en: "Refined sugar, ultra-processed snacks, late dinners", hi: "परिष्कृत चीनी, अत्यधिक प्रसंस्कृत स्नैक्स, देर रात का भोजन" },
          },
          {
            title: { en: `Report 2 of 4 — Partner / spouse (adult, 25–55)`,
                     hi: `4 में से रिपोर्ट 2 — साथी / जीवनसाथी (वयस्क, 25-55)` },
            ageBand: { en: "Adult 25–55", hi: "वयस्क 25-55" },
            calories: "1,800–2,400 kcal",
            protein: { en: "1.0–1.2 g/kg body wt", hi: "1.0-1.2 g/kg शरीर वजन" },
            focus: { en: "Iron (women), B12, calcium 1,000 mg, omega-3", hi: "आयरन (महिलाएं), B12, कैल्शियम 1,000 mg, ओमेगा-3" },
            avoid: { en: "Skipping meals, trans fats, excess salt (>5 g/day)", hi: "भोजन छोड़ना, ट्रांस वसा, अधिक नमक (>5 g/दिन)" },
          },
          {
            title: { en: `Report 3 of 4 — Child or teen #1 (5–19)`,
                     hi: `4 में से रिपोर्ट 3 — बच्चा या किशोर #1 (5-19)` },
            ageBand: { en: "Child / Teen 5–19", hi: "बच्चा / किशोर 5-19" },
            calories: "1,400–2,400 kcal",
            protein: { en: "1.0–1.5 g/kg body wt", hi: "1.0-1.5 g/kg शरीर वजन" },
            focus: { en: "Calcium 800–1,300 mg, Iron 8–18 mg, D3 600 IU, daily milk/curd", hi: "कैल्शियम 800-1,300 mg, आयरन 8-18 mg, D3 600 IU, दैनिक दूध/दही" },
            avoid: { en: "Critical: never calorie-restrict; avoid sugary drinks and skipping breakfast", hi: "महत्वपूर्ण: कैलोरी सीमित न करें; मीठे पेय और नाश्ता छोड़ने से बचें" },
          },
          {
            title: { en: `Report 4 of 4 — Senior member (55+)`,
                     hi: `4 में से रिपोर्ट 4 — वरिष्ठ सदस्य (55+)` },
            ageBand: { en: "Senior 55+", hi: "वरिष्ठ 55+" },
            calories: "1,600–2,100 kcal",
            protein: { en: "Critical: 1.2–1.5 g/kg (prevent sarcopenia)", hi: "महत्वपूर्ण: 1.2-1.5 g/kg (सार्कोपेनिया रोकें)" },
            focus: { en: "Calcium 1,200 mg + D3 2,000 IU, B12 1,000 mcg, omega-3 1 g", hi: "कैल्शियम 1,200 mg + D3 2,000 IU, B12 1,000 mcg, ओमेगा-3 1 g" },
            avoid: { en: "Protein restriction (drives muscle loss), low-fat dogma", hi: "प्रोटीन सीमा (मांसपेशी हानि), कम-वसा हठ" },
          },
        ]
      : [
          {
            title: { en: `Report 1 of 2 — Primary adult: ${profile.name} (age ${profile.age})`,
                     hi: `2 में से रिपोर्ट 1 — प्रमुख वयस्क: ${profile.name} (आयु ${profile.age})` },
            ageBand: { en: "Adult 20–45", hi: "वयस्क 20-45" },
            calories: `${adultCalLow}–${adultCalHigh} kcal`,
            protein: { en: `${Math.round((profile.weightKg || 65) * 1.6)}–${Math.round((profile.weightKg || 65) * 2.0)} g/day`,
                       hi: `${Math.round((profile.weightKg || 65) * 1.6)}–${Math.round((profile.weightKg || 65) * 2.0)} g/दिन` },
          ...
          {
            title: { en: `Report 2 of 2 — Partner, child, or parent (one extra household member)`,
                     hi: `2 में से रिपोर्ट 2 — साथी, बच्चा या माता-पिता (एक अतिरिक्त सदस्य)` },
            ageBand: { en: "Adult / senior 25–60+", hi: "वयस्क / वरिष्ठ 25-60+" },
            calories: "1,600–2,400 kcal",
            protein: { en: "1.0–1.5 g/kg body wt", hi: "1.0-1.5 g/kg शरीर वजन" },
            focus: { en: "Calcium 1,000–1,200 mg, B12, D3 1,500 IU, iron (women)", hi: "कैल्शियम 1,000-1,200 mg, B12, D3 1,500 IU, आयरन (महिलाएं)" },
            avoid: { en: "Skipping meals, trans fats, excess salt", hi: "भोजन छोड़ना, ट्रांस वसा, अधिक नमक" },
          },
        ];
    // Emit a personalised mini-report per purchased member slot — same
    // age-band guidance is materialised as a per-report block so the
    // bundle quantity (2 vs 4) is reflected as N distinct deliverables
    // within the single household PDF.
    for (const slot of memberSlots) {
      addText(ctx, `▸ ${language === "hi" ? slot.title.hi : slot.title.en}`, 9, DARK);
      drawPremiumTable(ctx,
        [l("Field","क्षेत्र"), l("Personalised Target","व्यक्तिगत लक्ष्य")],
        [
          [l("Age band","आयु वर्ग"), l(slot.ageBand.en, slot.ageBand.hi)],
          [l("Daily calories","दैनिक कैलोरी"), slot.calories],
          [l("Protein target","प्रोटीन लक्ष्य"), l(slot.protein.en, slot.protein.hi)],
          [l("Critical nutrients","महत्वपूर्ण पोषक तत्व"), l(slot.focus.en, slot.focus.hi)],
          [l("AVOID","बचें"), l(slot.avoid.en, slot.avoid.hi)],
        ],
        [40, 160],
      );
      addSpacing(ctx, 2);
    ...
    addText(
      ctx,
      l(
        `${memberCount} personalised mini-reports above are derived from this household profile. Send the names + ages of the additional ${memberCount - 1} member(s) to support@genewell.in within 7 days to receive their fully individualised PDFs.`,
        `ऊपर दी गई ${memberCount} व्यक्तिगत मिनी-रिपोर्ट इस घरेलू प्रोफ़ाइल से प्राप्त की गई हैं। अतिरिक्त ${memberCount - 1} सदस्य(ओं) के नाम + आयु 7 दिनों के भीतर support@genewell.in पर भेजें।`,
      ),
      8,
      DARK,
    );
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        // Skip dairy-, gluten-, and egg-heavy shared-meal rows when contraindicated.
        ((profile.foodIntolerances || []).some(f => /lactose|dairy|gluten|wheat/i.test(f.trim())) ? [] : [[
        ...
        ((profile.foodIntolerances || []).some(f => /^eggs?$/i.test(f.trim())) ? [] : [[
          l("Eggs + Oats","अंडे + ओट्स"),
          filterFoodLine(l("1 whole egg + 2 whites. Oats with milk + banana.","1 पूरा अंडा + 2 सफेद। दूध + केले के साथ ओट्स।"), profile.foodIntolerances),
          filterFoodLine(l("2 whole eggs for cholesterol balance. Oats with hot milk.","कोलेस्ट्रॉल संतुलन के लिए 2 पूरे अंडे। गर्म दूध के साथ ओट्स।"), profile.foodIntolerances),
    ...
    {
      const fi = profile.foodIntolerances;
      const enExtras = filterFoodLine("eggs, paneer", fi);
      const hiExtras = filterFoodLine("अंडे, पनीर", fi);
      drawGoldCallout(ctx,
        l("Family Grocery Budget — Weekly Optimization (India)", "परिवार किराना बजट — साप्ताहिक अनुकूलन (भारत)"),
        l(`Family of 4: ₹2,500–₹3,500/week covers all fresh vegetables, dal, rice/millets, ${enExtras}, seasonal fruits. Buy: dals in bulk (saves 20–30%), seasonal produce (saves 25–40%), frozen vegetables in off-season. AVOID: ultra-processed snacks, branded cereals (2–3× expensive, nutrient-poor).`,
          `4 लोगों का परिवार: ₹2,500–₹3,500/सप्ताह में सभी ताजी सब्जियां, दाल, चावल/बाजरा, ${hiExtras}, मौसमी फल शामिल हैं। खरीदें: थोक में दालें (20-30% बचत), मौसमी उत्पाद (25-40% बचत)। बचें: अत्यधिक प्रसंस्कृत स्नैक्स, ब्रांडेड अनाज (2-3× महंगे, पोषण-कमज़ोर)।`)
      );
    }
  ...
  if (addOns.includes("addon_women_hormone") && isFemale) {
    ...
    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonWomenHormoneIntro", decisions.variants.addonWomenHormoneIntro, language, {
      body: l(`Female hormones follow a 28-day cycle across 4 distinct phases. This section gives you phase-specific CALORIE ADJUSTMENTS, TRAINING RECOMMENDATIONS, and SUPPLEMENT TIMING that are completely absent from the general plan — because they apply only to you.`,
             `महिला हार्मोन 4 अलग-अलग चरणों में 28 दिन के चक्र का पालन करते हैं — चरण-विशिष्ट कैलोरी, प्रशिक्षण और सप्लीमेंट टाइमिंग।`),
    });
      ...
      if (conditions.includes("pcos")) {
        condRows.push(
          ...
          ["PCOS · Androgen Reduction",
            l("Spearmint tea 2×/day (clinical evidence supports reduction of free testosterone). Reduce dairy.","स्पियरमिंट चाय 2×/दिन (नैदानिक साक्ष्य मुक्त टेस्टोस्टेरोन में कमी का समर्थन करता है)। डेयरी कम करें।"),
      ...
      if (conditions.includes("thyroid")) {
        condRows.push(
          ["Thyroid · T4→T3 Support",
            (profile.foodIntolerances || []).some(f => /^nuts?$/i.test(f.trim()))
              ? l("Selenium 200mcg/day (CRITICAL — converts T4→T3). Use selenomethionine supplement.","सेलेनियम 200mcg/दिन (महत्वपूर्ण — T4→T3 रूपांतरित करता है)। सेलेनोमेथियोनिन सप्लीमेंट का उपयोग करें।")
  ...
  if (addOns.includes("addon_men_fitness") && isMale) {
    ...
    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonMenFitnessIntro", decisions.variants.addonMenFitnessIntro, language, {
      body: l(`This section gives you the testosterone optimization science + body composition targets that are ABSENT from your core movement plan. Calorie surplus computed from YOUR TDEE (${tdee} kcal), protein targets from YOUR weight (${profile.weightKg}kg).`,
             `यह अनुभाग आपको टेस्टोस्टेरोन अनुकूलन विज्ञान देता है जो मूल योजना में अनुपस्थित है।`),
    });
    ...
    drawPremiumTable(ctx,
      ...
      [
        [l("Sleep","नींद"),
          ...
          l("7–9 hrs","7-9 घंटे"),
          sleepHrs < 6
            ? l("CRITICAL: Research links very short sleep to lower testosterone. Fix sleep before all else.","गंभीर: शोध बहुत कम नींद को कम टेस्टोस्टेरोन से जोड़ता है। पहले नींद ठीक करें।")
            : l("Add 30 min sleep. Dark room, no screens 1 hr before bed.","30 मिनट नींद जोड़ें। अंधेरा कमरा, सोने से 1 घंटे पहले स्क्रीन नहीं।"),
          l("★CRIT★ Short sleep suppresses testosterone","★CRIT★ कम नींद टेस्टोस्टेरोन दबाती है")],
        [l("Body Fat","शरीर वसा"),
      ...
      [
        ["Ashwagandha KSM-66",
          ...
          (profile.stressScore || 50) > 50
            ? l(`★CRIT★ Stress ${stressLevel} suppresses LH. KSM-66 is the most studied extract for cortisol and testosterone support (JISSN 2021 — individual results vary).`,`★CRIT★ तनाव ${stressLevel} LH को दबाता है। KSM-66 कोर्टिसोल और टेस्टोस्टेरोन समर्थन के लिए सबसे अधिक अध्ययन किया गया एक्सट्रैक्ट है।`)
            : l("Cortisol → testosterone ratio. KSM-66 is clinically studied for testosterone support — individual results vary.","कोर्टिसोल → टेस्टोस्टेरोन अनुपात। KSM-66 का नैदानिक अध्ययन किया गया है — व्यक्तिगत परिणाम भिन्न होते हैं।"),
  ...
  if (addOns.includes("addon_products")) {
    ...
    addText(ctx,
      l(`Every product below is matched to YOUR specific deficiencies, goals, and conditions — NOT a generic list. Each product includes: what to buy, exact dose, when to take it, and WHY it is specifically relevant to your profile. All prices are verified Amazon India / HealthKart / PharmEasy as of 2025–2026.`,
        `नीचे प्रत्येक उत्पाद आपकी विशिष्ट कमियों, लक्ष्यों और स्थितियों से मेल खाता है।`),
      8.5, DARK
    );
    addSpacing(ctx, 4);
    ...
    if ((profile.sleepScore || 70) < 75) {
      productRows.push([
        ...
        l(`You reported ${profile.inputs?.sleepHoursLabel ?? "disrupted sleep"}. Magnesium bisglycinate is the most bioavailable form for sleep support and promotes GABA — the sleep neurotransmitter. Citrate and oxide forms are less effective for this purpose.`,
          `आपने ${profile.inputs?.sleepHoursLabel ?? "कम नींद"} बताई। मैग्नीशियम बिसग्लाइसिनेट GABA — नींद न्यूरोट्रांसमीटर — को बढ़ावा देता है।`),
    ...
    // Algae-based omega-3 for seafood-intolerant or vegetarian users.
    // never fish-oil. Previously this row was unconditional and surfaced "Fish Oil" to
    
    const _seaIntolP = (profile.foodIntolerances || []).some(f =>
      /seafood|^fish$|^fish_/i.test(f.trim()));
    const _algae = isVeg || _seaIntolP;
    productRows.push([
      _algae
        ? l("Omega-3 Algae Oil (EPA+DHA)", "ओमेगा-3 शैवाल तेल (EPA+DHA)")
      ...
      _algae
        ? "Nordic Naturals Algae Omega / Deva Vegan Omega (Amazon India)"
    ...
    if ((profile.stressScore || 50) > 45) {
      productRows.push([
        ...
        l(`Your stress is ${stressLevel}. KSM-66 is the most clinically studied Ashwagandha extract (24+ trials) for cortisol and immune support. Consistent use over several weeks is needed to assess your personal response.`,
          `आपका तनाव ${stressLevel} है। KSM-66 कोर्टिसोल और प्रतिरक्षा समर्थन के लिए सबसे अधिक अध्ययन किया गया अश्वगंधा एक्सट्रैक्ट है।`),
    ...
    if (conditions.includes("thyroid")) {
      productRows.push([
        ...
        l(`THYROID-SPECIFIC: Selenium is required for the enzyme that converts T4→T3. Your thyroid condition means this conversion may be suboptimal. Selenomethionine is better absorbed than selenite.${(profile.foodIntolerances || []).some(f => /^nuts?$/i.test(f.trim())) ? "" : " Brazil nuts (1/day) are an alternative."}`,
    ...
    if (profile.skinConcerns?.some((s: string) => s.toLowerCase().includes("acne"))) {
      productRows.push([
        ...
        l(`ACNE-SPECIFIC: Zinc reduces sebum production and IGF-1 (key acne driver). Spearmint reduces DHT and free testosterone — the hormonal driver of adult acne. Clinical evidence supports improvement in hormonal acne with consistent use; individual results vary.`,
          `मुहांसे-विशिष्ट: जिंक सीबम उत्पादन और IGF-1 कम करता है। स्पियरमिंट DHT और मुक्त टेस्टोस्टेरोन कम करता है; निरंतर उपयोग से सुधार का नैदानिक साक्ष्य है।`),
  ...
  // ════════════════════════════════════════════════════════════
  // ADD-ON H · QUARTERLY LAB-TEST INTERPRETATION
  // Phase 3 — content-block driven; no static tables, intro carries
  // the value proposition + bullets. Reorder/expand later as needed.
  // ════════════════════════════════════════════════════════════
  if (addOns.includes("addon_lab_interpretation")) {
    addNewPage(ctx);
    drawSectionBanner(ctx,
      l("H · QUARTERLY LAB-TEST INTERPRETATION", "H · त्रैमासिक लैब-टेस्ट व्याख्या"),
      l("Personalized Lab Panel Review + Action Triggers", "व्यक्तिगत लैब पैनल समीक्षा + कार्रवाई ट्रिगर"),
      l(`${profile.name} · ${profile.age} yrs · ${genderDisplay} — Quarterly review tied to your goals and conditions`,
        `${profile.name} · ${profile.age} वर्ष · ${genderDisplay} — आपके लक्ष्यों और स्थितियों से जुड़ी त्रैमासिक समीक्षा`)
    );

    addSpacing(ctx, 3);
    renderAddonIntroBlock(ctx, "addonLabInterpretation", decisions.variants.addonLabInterpretation, language, {
      body: l(`Every 3 months, upload your lab results and receive a structured interpretation tied to YOUR profile — not a generic reference range. Each marker is mapped to one of three states: on-track, watch, or act-now, with the exact next step.`,
             `हर 3 महीने में, अपने लैब परिणाम अपलोड करें और आपके प्रोफ़ाइल से जुड़ी संरचित व्याख्या प्राप्त करें।`),
      bullets: [
        l("Quarterly cadence with reminders aligned to your plan phases.", "आपकी योजना चरणों के साथ संरेखित त्रैमासिक अनुस्मारक।"),
        l("Each marker tagged on-track / watch / act-now with the exact next step.", "प्रत्येक मार्कर on-track / watch / act-now टैग के साथ।"),
        l("Trends across quarters so changes — not single readings — drive decisions.", "तिमाहियों में रुझान ताकि बदलाव — एकल रीडिंग नहीं — निर्णय लें।"),
      ],
    });

    renderLabInterpretationDetails(ctx, profile, language);

    addNewPage(ctx);
  }
}

function renderLabInterpretationDetails(
  ctx: PDFContext,
  profile: WellnessUserProfile,
  language: "en" | "hi",
): void {
  const l = (en: string, hi: string) => language === "hi" ? hi : en;
  const grid = buildLabInterpretationGrid(profile);

  const statusBadge = (s: LabPanelRow["status"]): string => {
    if (s === "act_now") return `★CRIT★ ${l("ACT NOW", "अभी कार्रवाई")}`;
    if (s === "watch")   return `★WARN★ ${l("WATCH", "निगरानी")}`;
    return `★GOOD★ ${l("ON TRACK", "सही दिशा")}`;
  };

  // ── Panel-by-panel status grid ───────────────────────────────────
  addSpacing(ctx, 2);
  addSubSection(ctx, l("Panel-by-Panel Status Grid", "पैनल-दर-पैनल स्थिति ग्रिड"));
  addText(ctx,
    l(`This grid converts your profile into a starting status for each of the seven standard panels. When you upload real lab results next quarter the engine will recompute the same grid against your actual numbers — same shape, same actions, just driven by data instead of profile signals.`,
      `यह ग्रिड आपकी प्रोफ़ाइल को सात मानक पैनलों के लिए शुरुआती स्थिति में बदलता है। अगली तिमाही में वास्तविक लैब परिणाम अपलोड करने पर इंजन उसी आधार पर पुनर्गणना करेगा।`),
    8.5, DARK);
  addSpacing(ctx, 1);
  ...
  drawPremiumTable(
    ...
    [
      ...
      l("Reference", "संदर्भ"),
      l("Status", "स्थिति"),
      l("Why · Next Step", "क्यों · अगला कदम"),
    ],
    grid.map(row => [
      row.panel,
      row.marker,
  ...
  addSpacing(ctx, 4);
  addSubSection(ctx, l("Quarterly Trend Snapshot", "त्रैमासिक रुझान स्नैपशॉट"));
  addText(ctx,
    l(`Bring four data points per marker (baseline → Q1 → Q2 → Q3) into this template. Decisions are made on the trend column, not on any single reading — a single high HbA1c after a holiday week is noise; three rises in a row is a signal.`,
      `प्रत्येक मार्कर के लिए चार डेटा बिंदु लाएं (आधार → Q1 → Q2 → Q3)। निर्णय रुझान पर लिए जाते हैं, किसी एक रीडिंग पर नहीं।`),
    8.5, DARK);
  addSpacing(ctx, 1);

  // Pick the 4 most-actionable rows for the trend table to keep the page balanced.
  const trendRows = [...grid]
    .sort((a, b) => {
      const order: Record<LabPanelRow["status"], number> = { act_now: 0, watch: 1, good: 2 };
  ...
  drawPremiumTable(
    ...
    [
      ...
      l("Q2", "Q2"),
      l("Q3", "Q3"),
      l("Starting Status", "शुरुआती स्थिति"),
    ],
    trendRows,
    [40, 22, 22, 22, 22, 58],
  );

  // ── What to do this quarter — goal-anchored prose + callout ─────
  addSpacing(ctx, 4);
  addSubSection(ctx, l("What to Do This Quarter", "इस तिमाही क्या करें"));

  const actNow   = grid.filter(r => r.status === "act_now");
  const watch    = grid.filter(r => r.status === "watch");
  const goalsRaw = (profile.goals || []).join(", ").trim();
  const goalLine = goalsRaw
    ? l(`Your stated goals — ${goalsRaw} — anchor the priorities below.`,
        `आपके बताए गए लक्ष्य — ${goalsRaw} — नीचे की प्राथमिकताओं को निर्धारित करते हैं।`)
    : l(`Your maintenance goal anchors the priorities below.`,
        `आपका रखरखाव लक्ष्य नीचे की प्राथमिकताओं को निर्धारित करता है।`);

  addText(ctx, goalLine, 8.5, DARK);
  addSpacing(ctx, 1);

  if (actNow.length > 0) {
    addText(ctx,
      l(`ACT-NOW this quarter (${actNow.length}): ${actNow.map(r => r.panel).join(", ")}. Book these tests in the first two weeks so you have results before Week 4 of your plan.`,
        `इस तिमाही ACT-NOW (${actNow.length}): ${actNow.map(r => r.panel).join(", ")}। पहले दो सप्ताह में बुक करें।`),
      8.5, DARK);
  }
  if (watch.length > 0) {
    addText(ctx,
      l(`WATCH this quarter (${watch.length}): ${watch.map(r => r.panel).join(", ")}. Bundle into a single draw between Week 6–8 to track 6-week progress against your habits.`,
        `इस तिमाही निगरानी (${watch.length}): ${watch.map(r => r.panel).join(", ")}। सप्ताह 6-8 के बीच एक ही ड्रॉ में।`),
      8.5, DARK);
  }
  if (actNow.length === 0 && watch.length === 0) {
    addText(ctx,
      l(`All seven panels start the quarter ON-TRACK. One baseline draw between Week 4–6 is enough; we will only escalate if numbers move.`,
        `सभी सात पैनल इस तिमाही सही दिशा में हैं।`),
      8.5, DARK);
  }

  addSpacing(ctx, 2);
  const focusRow = actNow[0] ?? watch[0] ?? grid[0];
  drawGoldCallout(
    ctx,
    l(`Your #1 Lab-Linked Action: ${focusRow.panel}`,
      `आपकी #1 लैब-लिंक्ड क्रिया: ${focusRow.panel}`),
    l(`${focusRow.rationale} ${focusRow.action}`,
      `${focusRow.rationale} ${focusRow.action}`),
  );

  addSpacing(ctx, 2);
  addNote(ctx, l(
    "Where to test in India: Thyrocare / Healthians for home-collection bundles (CBC + CMP + lipid + thyroid + HbA1c + ferritin + 25-OH D typically Rs.1,500–2,500). Always fast 10–12 hours before the metabolic / lipid / HbA1c draw. Do not start a new supplement in the 7 days before testing.",
    "भारत में कहाँ परीक्षण कराएं: Thyrocare / Healthians के होम-कलेक्शन बंडल (आमतौर पर Rs.1,500-2,500)। चयापचय / लिपिड / HbA1c ड्रॉ से पहले 10-12 घंटे उपवास करें।",
  ));
}
...
export async function generatePDFFromBundle(rawBundle: PDFDataBundle, language: "en" | "hi" = "en"): Promise<{ blob: Blob; filename: string }> {
  ...
  // Phase 3 — always invoke renderAddOns so tier-bundled (e.g. Pro+
  // Product Recommendations) and auto-included (e.g. Women's Hormonal
  // for PCOS/thyroid/(peri)menopausal markers) sections are produced
  // even when the user purchased zero paid add-ons. renderAddOns
  // recomputes the effective set internally and early-returns when
...
export function convertLegacyToBundle(data: PersonalizationData, options: PDFGenerationOptions): PDFDataBundle {
  const { profile, insights } = data;
  const { tier, addOns = [], addOnScope, orderId, timestamp } = options;
  ...
  const wellnessProfile: WellnessUserProfile = {
    ...
    wakeUpTime: profile.wakeUpTime || "6-8",
  };
  ...
  if (hasMedical) {
    ...
    if (conditions.some(c => c.includes("thyroid"))) {
      {
        const _nutFree = (wellnessProfile.foodIntolerances || []).some(f => /^nuts?$/i.test(f.trim()));
        conditionNarratives["thyroid"] = _nutFree
          ? "Thyroid Support: Prioritize selenium (selenomethionine supplement) and iodine (if cleared by doctor). Avoid excessive raw cruciferous vegetables."
          : "Thyroid Support: Prioritize selenium-rich foods (brazil nuts) and iodine (if cleared by doctor). Avoid excessive raw cruciferous vegetables.";
      }
    }
    ...
    if (conditions.some(c => c.includes("diabetes") || c.includes("sugar"))) {
      ...
      conditionNarratives["diabetes"] = "Glycemic Control: Pair all carbohydrates with fiber, protein, and healthy fats to blunt insulin spikes. Post-meal 10-minute walks are mandatory."
        + (saltyCraving ? ` ⚠️ Salty cravings detected: Excess sodium worsens insulin resistance and raises blood pressure. Replace namkeen/chips with ${filterFoodLine("roasted makhana, cucumber slices with rock salt, a small handful of unsalted nuts", wellnessProfile.foodIntolerances)}.` : "");
    }
    if (conditions.some(c => c.includes("hypertension") || c.includes("blood pressure"))) {
      conditionNarratives["hypertension"] = `Blood Pressure Management: Reduce sodium to <2,300mg/day, increase potassium-rich foods. You reported ${profile.inputs?.stressLevelLabel ?? stressLevel} stress — ${profile.stressScore < 60 ? "stress management may help alongside dietary changes." : "continue stress management practices alongside dietary changes."}`;
    }
  ...
  if (isPremiumOrAbove(tier)) {
    ...
    dayNames.forEach(dayLabel => {
      ...
      // default seed meals now route through filterFoodLine in
      // allow-empty mode so we can substitute a structured meal instead of the
      // generic "seasonal whole foods" placeholder. Eg a seafood-intolerant
      // non-veg user previously got "Fish curry + roti" as a default dinner —
      // now they get "Grilled chicken + roti + steamed vegetables".
      const fi2 = wellnessProfile.foodIntolerances;
      const safe = (primary: string, fallback: string) => {
        const filtered = filterFoodLine(primary, fi2, { allowEmpty: true });
        return filtered && filtered.trim().length > 0 ? filtered : fallback;
      };
      const _bf = safe(isVeg ? "Masala oat porridge with almonds + banana" : "3 eggs scrambled + whole wheat toast", "Vegetable upma + fruit");
      const _mm = safe("Roasted makhana + green tea", "Roasted chickpeas + green tea");
      const _ln = safe(isVeg ? "Rajma curry + brown rice + salad" : "Grilled chicken + brown rice + dal + salad", "Dal + brown rice + salad");
      const _es = safe("Trail mix (nuts + seeds)", "Roasted seeds + fruit");
      const _dn = safe(isVeg ? "Paneer tikka + roti + palak sabzi" : "Grilled chicken + roti + steamed vegetables", "Dal + rice + vegetables");
      const day: DayMeal = {
        dayLabel,
        breakfast: [makeMeal(_bf, 350)],
        midMorningSnack: [makeMeal(_mm, 120)],
        lunch: [makeMeal(_ln, 550)],
        eveningSnack: [makeMeal(_es, 140)],
        dinner: [makeMeal(_dn, 500)],
