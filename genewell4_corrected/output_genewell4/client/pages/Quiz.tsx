...
const quizQuestions = [
  ...
  {
    // collect actual height — replaces hardcoded gender-based estimate (was 160/175 cm).
    id: "heightCm",
    title: "What's your height?",
    subtitle: "Used to calculate your BMR and BMI accurately (no more gender-based guess)",
    icon: Scale,
  ...
  {
    // collect actual weight — replaces hardcoded gender-based estimate (was 65/80 kg).
    id: "weightKg",
    title: "What's your current weight?",
    subtitle: "Calorie targets and macros are calculated from this",
    icon: Scale,
    type: "number" as const,
    min: 30,
    max: 200,
  ...
  {
    id: "energyPattern",
    title: "How does your energy flow through the day?",
    subtitle: "Combines when you feel tired and your overall energy level",
    icon: Zap,
    type: "select" as const,
    options: [
      { value: "consistent-high", label: "Consistently high all day", emoji: "🚀" },
      { value: "afternoon-dip", label: "Strong morning, afternoon crash", emoji: "🥱" },
      { value: "morning-fog", label: "Foggy morning, picks up later", emoji: "🌫️" },
      { value: "evening-crash", label: "OK till evening, then crash", emoji: "😮‍💨" },
      { value: "always-tired", label: "Tired most of the day", emoji: "🔋" },
    ],
  },
  {
    id: "dietaryPreference",
    title: "What's your dietary preference?",
    subtitle: "Determines food list and supplement form (e.g. fish vs algae omega-3)",
    icon: Utensils,
    type: "select" as const,
    options: [
      ...
      { value: "eggetarian", label: "Eggetarian (veg + eggs)", emoji: "🥚" },
    ],
  },
  {
    id: "stressLevel",
  ...
  {
    id: "gutSymptoms",
    title: "Which gut symptoms do you experience?",
    subtitle: "Select all that apply — combines bloating and digestive issues into one signal",
    icon: Activity,
    type: "multi" as const,
    options: [
      { value: "bloating", label: "Bloating after meals", emoji: "💨" },
      ...
      { value: "gas", label: "Excess gas", emoji: "🌬️" },
      { value: "none", label: "No gut issues", emoji: "✅" },
    ],
  },
  {
...
export default function Quiz() {
  ...
  const isStepValid = () => {
    ...
    // number questions (heightCm, weightKg) need numeric range validation
    if ((currentQuestion as any).type === "number") {
      const n = Number(currentAnswer);
      const min = (currentQuestion as any).min ?? -Infinity;
      const max = (currentQuestion as any).max ?? Infinity;
      return Number.isFinite(n) && n >= min && n <= max;
    }
    if ((currentQuestion as any).type === "multi") {
  ...
  const submitQuiz = async () => {
    ...
    try {
      const ageRange = quizData.age as unknown as string;
      ...
      const toArray = (v: unknown, fallback: string): string[] =>
        Array.isArray(v) ? v as string[] : v ? [v as string] : [fallback];

      const skinConcernsArray = toArray(rest.skinConcerns, "none");
      const foodIntolerancesArray = toArray(rest.foodIntolerances, "none");
      const gutSymptomsArray = toArray(rest.gutSymptoms, "none");
      ...
      const finalQuizData = {
        ...
        gutSymptoms: gutSymptomsArray,
        dietaryPreference: rest.dietaryPreference || "non-veg",
        energyPattern: rest.energyPattern || "afternoon-dip",
      ...
      <div className="flex-1 pb-28 sm:pb-8">
        ...
        <Card className="border-0 shadow-xl sm:shadow-2xl bg-white/90 backdrop-blur-sm overflow-hidden">
          ...
          <CardContent className="space-y-3 sm:space-y-6 px-3 sm:px-8 pt-2">
            ...
            {/* Multi-select questions (e.g. gutSymptoms) */}
            {(currentQuestion as any).type === "multi" && (
              <div className="grid grid-cols-1 gap-2 sm:gap-3">
                {currentQuestion.options.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (option.value === "none") {
                          ...
                          next = [...selected.filter((v) => v !== "none"), option.value];
                        }
                    ...
                    >
                      ...
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                        isChecked ? "border-purple-600 bg-purple-600" : "border-slate-300"
                      }`}>
            ...
            {/* Number input for height/weight questions */}
            {(currentQuestion as any).type === "number" && (
              <div className="space-y-3">
                <div className="relative">
                  <Input
                    ...
                    value={(quizData as any)[currentQuestion.id] ?? ""}
                    onChange={(e) => {
                      const v = e.target.value;
                      // Numeric quiz fields store a number when valid, or "" while empty.
                      // Both shapes are accepted by the partial WellnessQuiz state.
                      const next: number | "" = v === "" ? "" : Number(v);
                      handleAnswer(currentQuestion.id, next);
                    }}
                    className="h-14 text-lg pr-16 text-center font-semibold"
                    autoFocus
                  />
                ...
                <p className="text-[11px] sm:text-xs text-slate-500 text-center">
                  Range: {(currentQuestion as any).min}–{(currentQuestion as any).max} {(currentQuestion as any).unit}
                </p>
              </div>
            )}
