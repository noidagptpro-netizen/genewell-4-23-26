    ...
    try {
      ...
      const personalizationData: PersonalizationData = {
        profile: {
          ...
          // prefer raw quiz heightCm/weightKg before falling back to anything.
          // Hardcoded gender-based fallbacks (175/75 male, 160/65 female) made every
          // user of the same gender get identical macros — root cause of the inaccuracy bug.
          estimatedHeightCm: (ap.estimatedHeightCm > 0 && !isNaN(ap.estimatedHeightCm))
            ? ap.estimatedHeightCm
            : (Number(parsed.heightCm) > 0 ? Number(parsed.heightCm) : (ap.gender === "male" ? 175 : 160)),
          estimatedWeightKg: (ap.estimatedWeightKg > 0 && !isNaN(ap.estimatedWeightKg))
            ? ap.estimatedWeightKg
            : (Number(parsed.weightKg) > 0 ? Number(parsed.weightKg) : (ap.gender === "male" ? 75 : 65)),
