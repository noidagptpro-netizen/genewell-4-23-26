...
import {
  ...
  getAddOnById,
  PlanConfiguration,
  DEPRECATED_ADDON_IDS,
} from "@/lib/products";
import LegalFooter from "@/components/LegalFooter";
import { getStoredProfile } from "@/lib/addon-filter";
...
export default function Checkout() {
  ...
  // Phase 3 — flag tier-bundled & auto-included add-ons so the summary
  // labels them "Included free" instead of charging them again.
  const includedAddOnReasons: Map<string, string | undefined> = (() => {
    ...
    const eff = computeEffectiveAddOns([], profile, configuration.planId);
    const includedSet = new Set([...eff.tierBundled, ...eff.autoIncluded]);
  ...
  // Phase 3 — surface auto-included (e.g. Women's Hormonal for PCOS)
  // and tier-bundled (e.g. Product Recommendations on Pro+) add-ons as
  // explicit line items so the user sees exactly what their order
  // contains, not only what they manually selected.
  const selectedIds = new Set(configuration.selectedAddOns);
  const autoBundledAddOns = Array.from(includedAddOnIds.keys())
  ...
  const handlePayWithInstamojo = async () => {
    ...
    try {
      ...
      // Phase 3 — sanitise stale add-on ids (deprecated SKUs from old
      // localStorage configs) and enforce Family Plan exclusivity, then
      // recompute the authoritative total from the live catalog so the
      // amount we charge always matches what we actually bill for.
      const FAMILY_BUNDLE_IDS = ["addon_family_2", "addon_family_4"];
      const rawAddOnIds = configuration.selectedAddOns ?? [];
      const dedupedAddOnIds = rawAddOnIds.filter(
        (id) => !DEPRECATED_ADDON_IDS.includes(id),
      );
      const familyChoice = dedupedAddOnIds
        .filter((id) => FAMILY_BUNDLE_IDS.includes(id))
        .pop();
      const sanitizedAddOnIds = dedupedAddOnIds.filter(
        (id) => !FAMILY_BUNDLE_IDS.includes(id) || id === familyChoice,
      );
      const sanitizedAddOnPrice = sanitizedAddOnIds.reduce((sum, id) => {
        const a = getAddOnById(id);
        return sum + (a?.price ?? 0);
      }, 0);
      const sanitizedTotalPrice = (plan?.price ?? 0) + sanitizedAddOnPrice;
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...configuration,
          selectedAddOns: sanitizedAddOnIds,
      ...
      const response = await fetch("/api/payments/create-direct-payment-link", {
        ...
        body: JSON.stringify({
          ...
          // Phase 3 — sanitise stale/legacy add-on ids that may have
          // been persisted in localStorage from prior sessions. The
          // server rejects deprecated ids, so stripping them here
          // prevents a confusing checkout failure for returning users.
          addOns: sanitizedAddOnIds,
          // Phase 3 — forward per-add-on scope (family member count,
          // lab cadence, etc.) so the server can persist it alongside
          // the order. Optional and forward-compatible: legacy callers
          // omit it.
          addOnScope: configuration.addOnScope ?? {},
          amount: sanitizedTotalPrice,
          quizData,
                    ...
                    >
                      <div className="flex-1">
                        <h4 className="font-semibold text-slate-900">
                          {addon!.name}
                          {isIncluded && (
                            <Badge className="ml-2 bg-emerald-600">Included free</Badge>
                          )}
                        </h4>
                        {isIncluded && (
                      ...
                      <div className="text-right ml-4 flex-shrink-0">
                        <p className="font-semibold text-slate-900">
                          {isIncluded ? "Included" : `₹${addon!.price.toLocaleString("en-IN")}`}
                        </p>
                      </div>
                    </div>
                    );
          ...
          <Card className="mb-6 border-2 border-blue-600">
            ...
            <CardContent className="p-6">
              <div className="space-y-3 mb-6">
                ...
                {autoBundledAddOns.map((addon) => {
                  const includedReason = includedAddOnReasons.get(addon!.id);
                  return (
                  <div key={addon!.id} className="flex justify-between">
                    <span className="text-slate-600">
                      {addon!.name}
                      <span className="ml-2 text-xs text-emerald-700">Included with your plan</span>
                      <span className="block text-xs text-emerald-700">
                        {includedReason || "Included based on your plan or profile"}
                      </span>
                    </span>
