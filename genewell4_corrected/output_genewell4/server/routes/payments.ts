...
// Add-on validation (shared by both payment routes).
// 1. Gender restrictions (Phase 1+).
// 2. Deprecated add-ons that have been folded into a tier or split into bundles (Phase 3).
const DEPRECATED_ADDON_IDS = new Set<string>(["addon_products", "addon_family"]);
const DEPRECATED_MESSAGES: Record<string, string> = {
  addon_products: "Product Recommendations is no longer sold separately — it is included in Personalized Pro+ and above.",
  addon_family:   "Family Wellness Plan has been replaced by Family Plan — 2 Reports and Family Plan — 4 Reports.",
};
...
function validateGenderAddonCompat(gender: string | undefined, addOns: string[] | undefined): { error: string | null; errorCode?: string; rejectedAddon?: string } {
  if (!addOns || addOns.length === 0) return { error: null };

  // Phase 3 — Family Plan is a single bundle: reject simultaneous
  // selection of the 2-report and 4-report SKUs so order semantics
  // remain unambiguous.
  if (addOns.includes("addon_family_2") && addOns.includes("addon_family_4")) {
    return {
      error: "Please choose either the 2-report or the 4-report Family bundle, not both.",
      errorCode: "ADDON_FAMILY_EXCLUSIVITY",
      rejectedAddon: "addon_family_4",
    };
  }

  // Block deprecated add-ons regardless of gender.
  const deprecated = addOns.find(id => DEPRECATED_ADDON_IDS.has(id));
  if (deprecated) {
    return { error: DEPRECATED_MESSAGES[deprecated] ?? "This add-on is no longer available.", errorCode: "ADDON_DEPRECATED", rejectedAddon: deprecated };
  }

  if (!gender) return { error: null };
...
export const handleCreateDirectPaymentLink: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      // Create purchase record with pending status (Phase 3 — persist
      // structured per-add-on scope alongside the order).
      purchase = await createPurchase(user.id, analysisId, planId, addOns || [], amount, addOnScope || {});
...
export const handleCreatePaymentRequest: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      // Create purchase record (Phase 3 — persist add-on scope).
      purchase = await createPurchase(user.id, analysisId, planId, addOns || [], amount, addOnScope || {});
