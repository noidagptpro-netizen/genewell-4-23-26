  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS purchases (
        ...
        add_ons_scope JSONB DEFAULT '{}'::JSONB,
      ...
      -- Phase 3 forward-migration: add scope column on existing deployments.
      ALTER TABLE purchases
        ADD COLUMN IF NOT EXISTS add_ons_scope JSONB DEFAULT '{}'::JSONB;
    ...
    // Phase 3 — backfill the "Product Recommendations included" bullet
    // for existing rows that were seeded before Pro+ folded the SKU in.
    // Idempotent: only appends the bullet if it's not already present.
    await client.query(`
      UPDATE managed_products
      SET details = details || '["Curated Product Recommendations included (no separate add-on)"]'::jsonb
      WHERE plan_id IN ('premium_blueprint', 'coaching_blueprint')
        AND NOT (details::text LIKE '%Curated Product Recommendations included%')
    `);
    ...
    const addonSeeds = [
      ...
      { addon_id: 'addon_family_2', name: 'Family Plan — 2 Reports', description: 'Two personalized reports — typically a couple, or a parent + child — with shared-meal modifier table', features: JSON.stringify(['2 personalized reports (one per profile)', 'Age-stratified target adjustments', 'Shared-meal modifier table', 'One grocery list, two sets of needs', 'Shared vs. individual lab tests']), price: 199, original_price: 299, icon: 'users', page_count_addition: 4, sort_order: 3 },
      { addon_id: 'addon_family_4', name: 'Family Plan — 4 Reports', description: 'Four personalized reports for a full household with shared-meal modifier table and per-person targets', features: JSON.stringify(['4 personalized reports (one per profile)', 'Age-stratified targets per household member', 'Shared-meal modifier table', 'One shopping list, four nutrition profiles', 'Shared vs. individual lab tests']), price: 349, original_price: 499, icon: 'users', page_count_addition: 6, sort_order: 4 },
      ...
      { addon_id: 'addon_lab_interpretation', name: 'Quarterly Lab-Test Interpretation', description: 'Quarterly lab panel personalized to your profile, with each marker translated into a concrete adjustment to your plan', features: JSON.stringify(['Quarterly panel scoped to your conditions & goals', 'Each marker mapped to a plan adjustment (not generic ranges)', 'PCOS / thyroid / metabolic-specific interpretation paths', 'Indian lab pricing & where-to-test guidance', 'Re-test cadence and next-quarter targets']), price: 249, original_price: 449, icon: 'flask-conical', page_count_addition: 3, sort_order: 7 },
    ];
...
export async function createPurchase(
  ...
  addOns: string[],
  totalPrice: number,
  addOnScope: Record<string, unknown> = {},
) {
  const result = await query(
    'INSERT INTO purchases (user_id, analysis_id, plan_id, add_ons, add_ons_scope, total_price) VALUES ($1, $2, $3, $4, $5::jsonb, $6) RETURNING *',
    [userId, analysisId, planId, addOns, JSON.stringify(addOnScope ?? {}), totalPrice]
  );
  return result.rows[0];
}
